fish <- tibble::tribble(
   ~`ID`,            ~`Species`,             ~`Loc`,         ~`Size`, ~`Val`,                            ~`Time`, ~`Month`,
...
test <- fish %>% pull(Size) %>% factor(., levels = size_levels, ordered = TRUE)

levels(test)

fish2 <- fish %>%
  mutate(Feature = case_when(
    Size == "Largest (Fin)" ~ "Fin",
    Size == "Large (Fin)" ~ "Fin",
    Size == "Narrow" ~ "Narrow",
    TRUE ~ "None")) %>%
  mutate(Size = fct_relevel(
    fct_collapse(factor(Size),
                 XS  = "Smallest",
                 S   = "Small",
                 M   = "Medium",
                 L   = c("Large", "Large (Fin)"),
                 XL  = c("X Large", "Narrow"),
                 XXL = c("Largest", "Largest (Fin)")),
    size_levels, ordered = TRUE))
